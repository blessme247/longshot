...
## 2026-07-18 — Rename to Longshot + wallet fixes (CODE landed; deploy cutover deferred past tonight's settlement)

**Live-match safety note (most important):** this work happened at ~22:10 UTC while France v England (kickoff 21:00) was mid-match. Its commitment had already fired correctly at 21:03:31 from the running `underdog-worker` (5 leaves, root e4f6d740…, tx 2DUiZU…), and settlement was ~30 min away. Deploying a second cron-bearing worker on the shared KV would risk double-crediting at settlement. A `wrangler triggers deploy` had likely pushed crons onto a freshly-deployed `longshot-worker`, so I **deleted longshot-worker** to guarantee exactly one settling worker tonight. Deployed state is back to underdog-worker only (proven, will settle). **The deploy cutover to longshot-* is intentionally deferred** to after settlement completes (or tomorrow AM, well before the final's 17:00 window). Code is committed and reviewable now; nothing user-facing is deployed yet.

**Fix #2 root cause (session persistence):** the sessions are localStorage HMAC bearer tokens (not cookies — chosen originally for the cross-origin workers.dev split, which is more robust than SameSite juggling; the prompt's cookie hypotheses don't apply). The real bug: an AccountChip effect cleared the session whenever the wallet adapter reported `!connected` — which is the transient state on EVERY page load before autoConnect resolves. So every refresh wiped a valid session. Fix: the stored session is the sole source of truth for auth, independent of adapter connection state; it's cleared only by an explicit disconnect handler (clicking the address chip), never reactively. Refresh and tab-reopen now restore auth silently. 7-day TTL; refresh-on-use skipped (needs a signed round-trip, not worth it).

**Fix #1 (SIWS name):** signing prompt now reads "Longshot wants you to sign in…" instead of the workers.dev URL. Breaking auth change as expected — combined with a fresh SESSION_SECRET on the new worker (set at cutover), existing sessions invalidate and users re-sign once. No leaf/proof data affected.

**Rename scope (code):** web title + og/description meta, header wordmark (Long·shot), tagline ("when the longshot lands"), all `underdog:` localStorage keys → `longshot:` (resets guest ids and sessions — guests start fresh, acceptable), removed dead `isUnderdogPrice`, risk.ts comment. wrangler names → longshot-worker / longshot-web, ALLOWED_ORIGINS + .env.production API URL updated (applied at cutover).

**Intentionally NOT renamed:** leaf prefix `underdog.pick.v1` (permanent in published commitment hashes — comment added explaining the historical name and a future v2 realignment) and the paired on-chain memo `underdog:v1:` (protocol id, not branding). **Deferred under deadline (per allowance):** package names `@underdog/*` stay — renaming ripples through 15 files' imports + a workspace relink for no user-visible benefit; the built web bundle strips import specifiers, so this won't show in the acceptance grep. Will realign with the leaf v2 work.

**Old URL disposition:** to be decided at cutover — leaning delete-both-underdog-deployments (single canonical longshot-*) since the SIWS + fresh-secret changes make any old-bundle session invalid, so keeping the old site alive would mislead. Will note the final choice when the cutover lands.
